...
export function useTradeUpdates(
  ...
  const handleTradeUpdate = useCallback((data: TradeUpdateMessage) => {
    if (data.type === 'trades' && Array.isArray(data.trades) && data.trades.length > 0) {
      onTradeUpdate(data.trades);
    }
  }, [onTradeUpdate]);

  useEffect(() => {
    if (chainId && tokenAddress) {
      console.log(`[useTradeUpdates] Subscribing to trades for ${tokenAddress} on ${chainId}`);
    ...
    return () => {
      if (unsubscribeRef.current) {
        console.log(`[useTradeUpdates] Unsubscribing from trades for ${tokenAddress || 'N/A'} on ${chainId || 'N/A'}`);
