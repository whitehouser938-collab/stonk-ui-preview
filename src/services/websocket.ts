...
class WebSocketManager {
  ...
  connect(): Promise<WebSocket> {
    return new Promise((resolve, reject) => {
      ...
      try {
        this.ws = new WebSocket(process.env.NEXT_PUBLIC_WEBSOCKET_URL || "ws://localhost:3003");
  ...
  subscribeBars(channel: string, onUpdate: (data: BarUpdateMessage) => void): () => void {
  ...
  subscribeTrades(address: string, chain: string, onUpdate: (data: TradeUpdateMessage) => void): () => void {
